( function ( $ ) {
        ...
        /**********************************************
         * Add to cart button From view
         * ********************************************/


        body.on( 'click', '.singleClickAddToCart', function ( e ) {
        e.preventDefault();
        e.stopPropagation();
        /**
         * Manual Function
        ...
        body.on( 'click', 'button.plus , a.plus , button.minus , a.minus', function ( e ) {
            ...
            if ($(this).attr("data-update-from-chart")){


                const quantityElem = $( this ).closest( '.quantity' ).find( '.qty_cart_js,.qty_pr_js' );

                let rowId    = quantityElem.attr('data-row-id')

                let quantityValue = $inputTag.val() ? parseInt( quantityElem.val(), 10 ) : 0;

                const quantity_text = $( this ).closest( '.mini_cart_item' ).find('.mini_cart_info').find('.cart_quantity_text');


                quantity_text.text('x' + quantityValue);


                updateCart(rowId,quantityValue);
...
function addItemToCartFromDetail(){
    let _token = $('meta[name="csrf-token"]').attr('content');
    //addCartItemFromVariantDetail()
    const product_name_for_cart  =$('#product_name').text();
    const variant_name_for_cart  = $('#variant_name').text();
    const variant_price_for_cart = $('#variant_price').text();
    ...
    $.ajax({
        ...
        data: {
            _token: _token,
            barcode: variant_barcode_for_cart,
            name:variant_name_for_cart,
            quantity:quantity_for_cart,
